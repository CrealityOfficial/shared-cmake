...
function(__add_real_target target type)
	...
	if(target_SOURCE)
		...
		elseif(${type} STREQUAL "dll")
			if(CC_GLOBAL_FORCE_STATIC AND NOT target_FORCE_DLL)
				add_library(${target} STATIC ${target_SOURCE} ${ExtraSrc})
				target_compile_definitions(${target} PRIVATE CC_FORCE_EXPORT_STATIC)
				message(STATUS "CC_GLOBAL_FORCE_STATIC ------------------------------> ${target} lib")
			else()
				add_library(${target} SHARED ${target_SOURCE} ${ExtraSrc})
				if(target_FORCE_DLL)
					target_compile_definitions(${target} PRIVATE CC_FORCE_IMPORT_STATIC)
				endif()
			endif()
